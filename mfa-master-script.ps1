<#
.SYNOPSIS
    Master controller for MFA Migration Assessment Tool - Enhanced Version with Path Management
...
function Show-Help {
    ...
    Write-Host "      • User MFA registration analysis (all 189 users)"
...
function Remove-WorkingDirectory {
    ...
    if ($pathInfo.UsedSymlink) {
        ...
        if (Test-Path $pathInfo.WorkingPath) {
            try {
                Remove-Item $pathInfo.WorkingPath -Force -ErrorAction SilentlyContinue
                Write-Host "Removed temporary working directory: $($pathInfo.WorkingPath)" -ForegroundColor Green
            }
            catch {
                Write-Warning "Could not remove temporary directory: $($pathInfo.WorkingPath)"
...
try {
    ...
    if ($GenerateReport -and $global:MFAAssessmentData.LegacyMfaData) {
        ...
        # Create Phase 2 automatic cleanup script
        $autoCleanupScript = @'
<#
.SYNOPSIS
    Automatically removes insecure authentication methods for users who have secure alternatives
    
.DESCRIPTION
    This script processes users identified in the "Auto-remove" group who have both
    secure and insecure authentication methods registered.
#>

param(
    [string]$UserListCsv = ".\MFA_User_Methods_*.csv",
    [switch]$WhatIf = $true,
    [int]$BatchSize = 50
)
...
# Load user list
$csvFile = Get-ChildItem $UserListCsv | Select-Object -First 1
if (-not $csvFile) {
    Write-Error "Cannot find user methods CSV file"
    return
}

$users = Import-Csv $csvFile.FullName | Where-Object { $_.Migration_Group -eq "Auto-remove" }
Write-Host "Found $($users.Count) users for automatic cleanup" -ForegroundColor Yellow
...
foreach ($user in $users) {
    ...
    $methodsToRemove = $user.Methods_To_Remove -split ","
    
    Write-Host "`nProcessing: $upn" -ForegroundColor Cyan
    Write-Host "  Methods to remove: $($methodsToRemove -join ', ')" -ForegroundColor Gray
    
    if (-not $WhatIf) {
        try {
            # Get user's authentication methods
            $userMethods = Get-MgUserAuthenticationMethod -UserId $upn
            
            foreach ($method in $methodsToRemove) {
                $methodToDelete = $null
                
                switch ($method.Trim()) {
                    "Phone" { $methodToDelete = $userMethods | Where-Object { $_.AdditionalProperties["@odata.type"] -eq "#microsoft.graph.phoneAuthenticationMethod" } }
                    "Email" { $methodToDelete = $userMethods | Where-Object { $_.AdditionalProperties["@odata.type"] -eq "#microsoft.graph.emailAuthenticationMethod" } }
                    "SMS" { $methodToDelete = $userMethods | Where-Object { $_.AdditionalProperties["@odata.type"] -eq "#microsoft.graph.smsAuthenticationMethod" } }
                }
                
                if ($methodToDelete) {
                    Remove-MgUserAuthenticationMethod -UserId $upn -AuthenticationMethodId $methodToDelete.Id
                    Write-Host "  Removed: $method" -ForegroundColor Green
                }
            }
        ...
        Write-Host "  [WHATIF] Would remove: $($methodsToRemove -join ', ')" -ForegroundColor Yellow
        $processed++
    }
    
    # Batch delay
    if ($processed % $BatchSize -eq 0) {
        Write-Host "Processed $processed users, pausing..." -ForegroundColor Gray
...
Write-Host "Total processed: $processed" -ForegroundColor Green
if ($errors.Count -gt 0) {
    Write-Host "Errors encountered: $($errors.Count)" -ForegroundColor Red
    $errors | Export-Csv ".\Cleanup_Errors_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv" -NoTypeInformation
}

if ($WhatIf) {
    Write-Host "`nThis was a WHAT IF run. Use -WhatIf:$false to execute changes." -ForegroundColor Yellow
}
'@
        
        $autoCleanupScript | Out-File -FilePath (Join-Path $automationPath "Phase2_AutoCleanup.ps1") -Encoding UTF8
        
        # Create privileged user FIDO2 report script
        $fidoReportScript = @'
<#
.SYNOPSIS
    Generates a report of privileged users needing FIDO2 security keys
#>

param(
    [string]$PrivilegedUsersCsv = ".\MFA_Privileged_Users_*.csv"
)

$csvFile = Get-ChildItem $PrivilegedUsersCsv | Select-Object -First 1
if (-not $csvFile) {
    Write-Error "Cannot find privileged users CSV file"
    return
}

$privUsers = Import-Csv $csvFile.FullName | Where-Object { $_.Requires_FIDO2 -eq "Yes" }

Write-Host "`nPRIVILEGED USERS REQUIRING FIDO2 SECURITY KEYS" -ForegroundColor Red
Write-Host "===============================================" -ForegroundColor Red
Write-Host "Total: $($privUsers.Count) administrators" -ForegroundColor Yellow

$privUsers | ForEach-Object {
    Write-Host "`n$($_.UserPrincipalName)" -ForegroundColor Cyan
    Write-Host "  Roles: $($_.All_Roles)" -ForegroundColor White
    Write-Host "  Current Methods: $($_.Methods)" -ForegroundColor Gray
    Write-Host "  Risk Level: $($_.Risk_Level)" -ForegroundColor $(if ($_.Risk_Level -eq "Critical") { "Red" } else { "Yellow" })
}

Write-Host "`n`nRECOMMENDED FIDO2 KEYS:" -ForegroundColor Green
Write-Host "- YubiKey 5 Series (USB-A or USB-C)"
Write-Host "- YubiKey Bio Series (Fingerprint)"
Write-Host "- Feitian BioPass"

Write-Host "`nORDER QUANTITY: $($privUsers.Count * 2) keys (2 per admin)" -ForegroundColor Yellow
'@
        
        $fidoReportScript | Out-File -FilePath (Join-Path $automationPath "Generate_FIDO2_Order.ps1") -Encoding UTF8
